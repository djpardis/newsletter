...
describe("footerBlock / footerText", () => {
  ...
  it("matches the canonical plain-text snapshot", () => {
    ...
    const text = footerText(env, "https://newsletter.example.com/api/unsubscribe?token=U");
    expect(text).toBe(
      [
        "",
        "",
        ...
        "",
        "---",
        "You're receiving this because you subscribed at Vance Refrigeration. To stop, unsubscribe here:",
  ...
  it("links the brand and unsubscribe in the HTML footer", () => {
    ...
    expect(block).toContain('<a href="https://example.com">Vance Refrigeration</a>');
...
describe("confirmation pages", () => {
  ...
  it("does not link the top brand label on the subscribe confirmation page", () => {
    ...
    expect(html).toContain(">Vance Refrigeration</p>");
