...
describe("campaign-content", () => {
  it("reads frontmatter and renders minimal HTML", () => {
    ...
    expect(campaign.html).toContain("— Future Shock Media<br>");
    expect(campaign.html).toContain("You&#39;re receiving this because you subscribed to Future Shock Media.<br>");
  ...
  it("rejects em-dash subject lines", () => {
    expect(validateCampaignSubject("Smoke test — Future Shock Media")).toContain(
      "Campaign subject must not use an em dash. Write it like a news update.",
    );
    expect(() =>
      renderCampaignMarkdown(goodCampaign.replace('subject: "Test campaign"', 'subject: "Smoke test — Future Shock Media"')),
